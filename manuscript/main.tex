...
%     An {\em architecture} on a ca an architecture $T$ is a natural transformation from $T$ to an endofunctor. More explicitly, given an architecture $T\colon \Cat \rightarrow \Cat$, a {\em locality constraint} is an endofunctor $S \colon \Cat \rightarrow \Cat$ together with a natural transformation $\eta \colon T \rightarrow S$.
% \end{definition}
